...
## Bonus. Reddit Topic Listener (Market Intelligence)
...
Watch a fixed set of subreddits for posts that mention a fixed keyword list, on a recurring schedule, and produce a weekly summary. Useful for **market intelligence on a fast-moving topic** (e.g. AI search, GEO, your category) — not brand monitoring, not outreach. It's topic listening.

The point: a small daily fetch + a weekly digest stops you from having to live on Reddit to keep up with a topic.

**Inputs**
- Subreddits: **[list — e.g. r/SEO, r/bigseo, r/SEO_LLM]**
- Topic keywords: **[fixed list — e.g. "AI search", "GEO", "LLMO", "AEO", "AI visibility", "AI overviews", "Perplexity", "ChatGPT search"]**
- Fetch cadence: **[daily / hourly]**
- Report cadence: **[weekly]**
- Storage: **[Postgres table / CSV in a Git repo / both]**

**Pipeline**
1. **Daily fetch (scheduled job)** — pull each subreddit's `.rss` feed, match every post against the keyword list, store matches permanently with `id, subreddit, title, content, author, url, published_at, matched_keywords`. Dedupe by Reddit post ID.
2. **Export checkpoint** — write the full table out as CSV and push to a Git repo. Backup + human-readable archive.
3. **Weekly LLM summary** — feed the last 7 days of matched posts to an LLM, ask for 3–5 themes heating up, representative threads per theme, and any new phrase worth adding to the keyword list.

**Output — weekly Markdown report**

```
# Reddit Topic Trends — Week of [date]

## Top themes this week
1. <theme>
   - Why: <1 sentence>
   - Threads: [Title](url) — <why>

## New phrases worth tracking
- "<phrase>" — appeared in <N> threads.

## Volume snapshot
- Matched posts: <N> (vs <N> last week)
- Most active subreddit: <name>
```

**Why this shape**
- Fixed subreddits + fixed keyword list keeps cost predictable and signal high. Open-ended scraping returns noise.
- Permanent storage turns Reddit into a queryable timeline.
- The weekly LLM digest is the part that turns a firehose into something you'll actually read.

Different shape from a brand-mention listener (which is one-off and outreach-focused). This one runs forever and tells you how a topic is moving.
